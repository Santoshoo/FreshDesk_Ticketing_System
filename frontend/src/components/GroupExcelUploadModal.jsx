...
export default function GroupExcelUploadModal({ isOpen, onClose, onSuccess }) {
  ...
  // 1. Download Sample Excel Template (Group Name and Status ONLY)
  const handleDownloadTemplate = () => {
    const sampleData = [
      {
        'Group Name': 'EMR & HIS Support',
        Status: 'ACTIVE',
      },
      {
        'Group Name': 'Network Infrastructure',
        Status: 'ACTIVE',
      },
      {
        'Group Name': 'Biomedical & Medical Devices',
        Status: 'ACTIVE',
      },
      {
        'Group Name': 'Hardware & Peripherals',
        Status: 'ACTIVE',
      },
      {
        'Group Name': 'PACS & Radiology IT',
        Status: 'ACTIVE',
      },
    ];
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Support_Groups');
    XLSX.writeFile(workbook, 'KIMS_Support_Groups_Import_Template.xlsx');
  };
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (cleaned.length === 0) {
          setError('No valid group records with a "Group Name" column were found.');
          setParsedGroups([]);
          return;
        }

        setParsedGroups(cleaned);
      } catch (err) {
  ...
  const handleImportSubmit = async () => {
    ...
    try {
      setLoading(true);
      setError('');
      const res = await groupApi.bulkCreate(parsedGroups);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Upload Support Groups (Excel Import)">
      <div className="space-y-5 text-xs">
        ...
        <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <p className="font-semibold text-slate-800">Super Admin Bulk Group Upload</p>
              <p className="text-[11px] text-slate-500 mt-0.5">
                Upload `.xlsx`, `.xls`, or `.csv` files containing support groups.
              </p>
            </div>
          </div>
        ...
        {resultSummary ? (
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 space-y-3">
            <div className="flex items-center gap-2 text-emerald-800 font-bold text-sm">
              ...
              <span>Bulk Group Import Completed!</span>
            </div>
            <p className="text-emerald-700">
              Successfully imported/updated <strong className="font-bold">{resultSummary.importedCount}</strong> support groups.
            </p>
          ...
          <>
            ...
            >
              ...
              <p className="text-[11px] text-slate-400 mt-1">
                Supports `.xlsx`, `.xls`, `.csv` with columns: <code className="bg-slate-100 px-1 py-0.5 rounded text-slate-600">Group Name</code>, <code className="bg-slate-100 px-1 py-0.5 rounded text-slate-600">Status</code>
              </p>
            </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-slate-800">
                    Preview Ready ({parsedGroups.length} groups found):
                  </p>
                  <button
                    type="button"
                    onClick={resetState}
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                disabled={loading || parsedGroups.length === 0}
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>Upload & Import {parsedGroups.length} Groups</span>
                  </>
                )}
              </button>
            </div>
